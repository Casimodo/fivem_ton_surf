...
-- RegisterCommand("ton_surf", function()
-- 	RCCar.Start()
-- end)


AddEventHandler('ton_surf:clear', function()
	RCCar.Clear()
end)

AddEventHandler('ton_surf:useItem', function()
	TriggerServerEvent('ton_surf:useItem')
    RCCar.Start()
end)
...
RCCar.HandleKeys = function(distanceCheck)
	if distanceCheck <= 1.5 then
		if IsControlJustPressed(0, 38) then
			TriggerServerEvent('ton_surf:unUseItem')
			RCCar.Attach("pick")
		end
...
RCCar.Spawn = function()
	-- models to load
	RCCar.LoadModels({ GetHashKey("SEASHARK"), 68070371, GetHashKey("p_defilied_ragdoll_01_s"), "pickup_object", "move_strafe@stealth", "move_crouch_proto"})
	...
	RCCar.Entity = CreateVehicle(GetHashKey("SEASHARK"), spawnCoords, spawnHeading, true)
	RCCar.Skate = CreateObject(GetHashKey("prop_surf_board_ldn_04"), 0.0, 0.0, 1.0, true, true, true)
	--RCCar.Skate = CreateObject(GetHashKey("prop_skid_trolley_2"), 0.0, 0.0, 1.0, true, true, true)
	...
	--AttachEntityToEntity(RCCar.Skate, RCCar.Entity, GetPedBoneIndex(PlayerPedId(), 28422), 0.0, 0.10, 0.04, -90.0, 0.0, 0.0, false, true, true, true, 1, true)
	AttachEntityToEntity(RCCar.Skate, RCCar.Entity, GetPedBoneIndex(PlayerPedId(), 28422), 0.0, 0.10, 0.03, -90.0, 0.0, 0.0, false, true, false, true, 1, true)
...
RCCar.AttachPlayer = function(toggle)
	...
	if toggle then
		...
		AttachEntityToEntity(player, RCCar.Entity, 20, 0.0, 0, 1.0, 0.0, 0.0, -15.0, true, true, false, true, 1, true)
...
RegisterNetEvent("ton_surf:shareHeIsOnSkate")
AddEventHandler("ton_surf:shareHeIsOnSkate", function(id) 
